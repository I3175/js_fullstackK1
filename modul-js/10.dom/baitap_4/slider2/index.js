...
const photoAllSelector = document.querySelectorAll('.photo_list > img');
let index = 0;
...
function handleNextButton() {
    ...
    for (let i = 0; i < photoAllSelector.length; i++){
        photoAllSelector[i].classList.remove('active');
...
function handlePreviousButton(){
    ...
    photoAllSelector[index].classList.add('active');
}
...
function handleActiveButton(event) {
    ...
    const clickedButtonIndex = Array.from(photoAllSelector).indexOf(event.target);
...
for (let i = 0; i < photoAllSelector.length; i++) {
    photoAllSelector[i].addEventListener('click', handleActiveButton);
}